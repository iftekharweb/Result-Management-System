...
const Students = () => {
  ...
  const filteredStudents =
    filter === "All"
      ? students
      : students.filter(
          (student) => student.semester.year === parseInt(filter)
        );

  const filteredBySID = searchSID
    ? filteredStudents.filter((student) =>
        student.SID.toString().includes(searchSID)
      )
    : filteredStudents;
    ...
    <div className="container mx-auto px-4 py-3">
      <div className="w-full flex flex-row justify-between">
        ...
        <div className="flex justify-center mb-4">
          <select
            className="p-2 mr-2 bg-gray-200 text-gray-800 rounded"
      ...
      <ul className="flex flex-col justify-start items-center w-full">
        ...
        {filteredBySID.map((student) => (
          <li
            key={student.id}
            className="flex flex-row justify-evenly items-center w-full rounded-md bg-gray-200 p-3 m-2"
          >
            <p className="text-[#060606] w-1/4 flex justify-center items-center">
              {student.SID}
            </p>
